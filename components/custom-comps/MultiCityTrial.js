...
import styles from "./MultiCityTrial.module.css";
...
const MultiCityTrial = ({ airports }) => {
  ...
  const onClickRoundTrip = () => {
    ...
    // );
    // window.localStorage.setItem('searchRows', JSON.stringify(searchRows));
        ...
        .then(async () => {
          ...
          await fetchFlightResultsMultiCity(
            ...
            .then((result) => {
              ...
              // } else {
              // console.log("MultiCity:", response);
              router.push(
                "/flight-match?flightRequestId=" +
                  result.data.response.requestId
              );
              // }
            })
            .catch((error) => {
