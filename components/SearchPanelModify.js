...
const SearchPanelModify = ({ airports, searchMeta }) => {

    // Set the router
    const router = useRouter();

    // Set the flight request
    const flightRequest = JSON.parse(searchMeta.flightRequest);

    // Set the search rows
    const _searchRows = JSON.parse(searchMeta.searchRows);
    console.log('Flight Request:', flightRequest);
    // Set the selected departure
    const selectedDeparture = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode1) :
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode);

    // Set the selected arrival
    const selectedArrival = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode1) :
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode);

    // Set the second selected departure
    const _selectedDeparture2 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode2) :
        null;

    // Set the second selected arrival
    const _selectedArrival2 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode2) :
        null;

    // Set the third selected departure
    const _selectedDeparture3 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode3) :
        null;

    // Set the third selected arrival
    const _selectedArrival3 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode3) :
        null;

    // Set the fourth selected departure
    const _selectedDeparture4 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode4) :
        null;

    // Set the fourth selected arrival
    const _selectedArrival4 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode4) :
        null;

    // Set the fifth selected departure
    const _selectedDeparture5 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originLocationCityCode5) :
        null;

    // Set the fifth selected arrival
    const _selectedArrival5 = searchMeta.routeModel == RouteModel.MultiCity ?
        airports.find(airport => airport.iataCode === flightRequest.originDestinationCityCode5) :
        null;

    // Search rows state
    const [searchRows, setSearchRows] = useState(searchMeta.routeModel == RouteModel.MultiCity ?
        _searchRows :
        [
            {
                id: 2,
    ...
    const [routeModel, setRouteModel] = useState(searchMeta.routeModel);
    const [tripValue, setTripValue] = useState(normalizeRouteModel(searchMeta.routeModel.toString()));
    const [optionVisible, setOptionVisible] = useState(false);

    // States for passengers section
    const [passengersVisible, setPassengersVisible] = useState(false);
    const [adultCount, setAdultCount] = useState(flightRequest.numberOfAdults);
    const [childrenCount, setChildrenCount] = useState(flightRequest.numberOfChildren);
    const [infantCount, setInfantCount] = useState(flightRequest.numberOfInfants);
    const [passengers, setPassengers] = useState(adultCount + childrenCount + infantCount);

    // States for class section
    const [classOptionsVisible, setClassOptionsVisible] = useState(false);
    const [classValue, setClassValue] = useState(deNormalizeClassValue(flightRequest.flightClass));
    const [flightClass, setFlightClass] = useState(flightRequest.flightClass);
    ...
    // Form data states 
    const [departureDate, setDepartureDate] = useState(new Date(searchMeta.routeModel == RouteModel.MultiCity ? flightRequest.departureDate1 : flightRequest.departureDate));
    const [arrivalDate, setArrivalDate] = useState(new Date(flightRequest.returningDate ? flightRequest.returningDate : new Date(moment(flightRequest.departureDate).add(1, 'days'))));
    ...
    const [originDepartureInputVal, setOriginDepartureInputVal] = useState(selectedDeparture.iataCode);
    const [originArrivalInputVal, setOriginArrivalInputVal] = useState(selectedArrival.iataCode);
    ...
    const [dateWindow, setDateWindow] = useState(flightRequest.dateWindow);

    //State for directFlightOnly
    const [directFlightOnly, setDirectFlightOnly] = useState(flightRequest.directFlightOnly);
    ...
    /**
     * Converts class value from Title case to Upper Case
     * @param {*} flightClass The flight class in Title case
     * @returns The Upper Case value
     */
    ...
    /**
     * Converts flight class from Upper Case to Title
     * @param {*} flightClass The flight class in upper case
     * @returns The Title case value
     */
    function deNormalizeClassValue(flightClass) {
        switch (flightClass) {
            case 'ECONOMY':
                return 'Economy';
            case 'BUSINESS':
    ...
    useEffect(() => {
        if (originDepartureInputVal != selectedDeparture?.iataCode) {
            // Set the value
    ...
    useEffect(() => {
        if (originArrivalInputVal != selectedArrival?.iataCode) {
            // Set the value
    ...
    // Form data states 
    const [secondDepartureDate, setSecondDepartureDate] = useState(new Date(searchMeta.routeModel == RouteModel.MultiCity ? flightRequest.departureDate2 : moment(departureDate).add(1, 'days')));
    ...
    const [secondOriginDepartureInputVal, setSecondOriginDepartureInputVal] = useState(_selectedDeparture2?.iataCode);
    const [secondOriginArrivalInputVal, setSecondOriginArrivalInputVal] = useState(_selectedArrival2?.iataCode);
    ...
    function autoSetSecondArrivalDate() {
        let date = departureDate > secondDepartureDate ? new Date(moment(departureDate).add(1, 'days')) : secondDepartureDate;
    ...
    // useEffect to focus on arrival input ref when origin departure input value has a value 
    useEffect(() => {
        if (secondOriginDepartureInputVal != _selectedDeparture2?.iataCode) {
            // Set the second origin location city code
            flightSearchCredentials.originLocationCityCode2 = secondOriginDepartureInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // useEffect to focus on departure date input ref when arrival input value has a value 
    useEffect(() => {
        if (secondOriginArrivalInputVal != _selectedArrival2?.iataCode) {
            // Set the second origin destination city code
            flightSearchCredentials.originDestinationCityCode2 = secondOriginArrivalInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // Form data states 
    const [thirdDepartureDate, setThirdDepartureDate] = useState(new Date(searchMeta.routeModel == RouteModel.MultiCity ? flightRequest.departureDate3 : moment(departureDate).add(1, 'days')));
    ...
    const [thirdOriginDepartureInputVal, setThirdOriginDepartureInputVal] = useState(_selectedDeparture3?.iataCode);
    const [thirdOriginArrivalInputVal, setThirdOriginArrivalInputVal] = useState(_selectedArrival3?.iataCode);
    ...
    function autoSetThirdArrivalDate() {
        let date = secondDepartureDate > thirdDepartureDate ? new Date(moment(secondDepartureDate).add(1, 'days')) : thirdDepartureDate;
    ...
    // useEffect to focus on arrival input ref when origin departure input value has a value 
    useEffect(() => {
        if (thirdOriginDepartureInputVal != _selectedDeparture3?.iataCode) {
            // Set the third origin location city code
            flightSearchCredentials.originLocationCityCode3 = thirdOriginDepartureInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // useEffect to focus on departure date input ref when arrival input value has a value 
    useEffect(() => {
        if (thirdOriginArrivalInputVal != _selectedArrival3?.iataCode) {
            // Set the third origin destination city code
            flightSearchCredentials.originDestinationCityCode3 = thirdOriginArrivalInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // Form data states 
    const [fourthDepartureDate, setFourthDepartureDate] = useState(new Date(searchMeta.routeModel == RouteModel.MultiCity ? flightRequest.departureDate4 : moment(departureDate).add(1, 'days')));
    ...
    const [fourthOriginDepartureInputVal, setFourthOriginDepartureInputVal] = useState(_selectedDeparture4?.iataCode);
    const [fourthOriginArrivalInputVal, setFourthOriginArrivalInputVal] = useState(_selectedArrival4?.iataCode);
    ...
    function autoSetFourthArrivalDate() {
        let date = thirdDepartureDate > fourthDepartureDate ? new Date(moment(thirdDepartureDate).add(1, 'days')) : fourthDepartureDate;
    ...
    // useEffect to focus on arrival input ref when origin departure input value has a value 
    useEffect(() => {
        if (fourthOriginDepartureInputVal != _selectedDeparture4?.iataCode) {
            // Set the fourth origin location city code
            flightSearchCredentials.originLocationCityCode4 = fourthOriginArrivalInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // useEffect to focus on departure date input ref when arrival input value has a value 
    useEffect(() => {
        if (fourthOriginArrivalInputVal != _selectedArrival4?.iataCode) {
            // Set the fourth origin destination city code
            flightSearchCredentials.originDestinationCityCode4 = fourthOriginArrivalInputVal;
            // Update the flight search credentials
            setFlightSearchCredentials(flightSearchCredentials);
            // Set focus on the next field
    ...
    // Form data states 
    const [fifthDepartureDate, setFifthDepartureDate] = useState(new Date(searchMeta.routeModel == RouteModel.MultiCity ? flightRequest.departureDate5 : moment(departureDate).add(1, 'days')));
    ...
    const [fifthOriginDepartureInputVal, setFifthOriginDepartureInputVal] = useState(_selectedDeparture5?.iataCode);
    const [fifthOriginArrivalInputVal, setFifthOriginArrivalInputVal] = useState(_selectedArrival5?.iataCode);
    ...
    function autoSetFifthArrivalDate() {
        let date = fourthDepartureDate > fifthDepartureDate ? new Date(moment(fourthDepartureDate).add(1, 'days')) : fifthDepartureDate;
    ...
    // useEffect to focus on arrival input ref when origin departure input value has a value 
    useEffect(() => {
        if (fifthOriginDepartureInputVal != _selectedDeparture5?.iataCode) {
            // Set the fifth origin location city code
    ...
    // useEffect to focus on departure date input ref when arrival input value has a value 
    useEffect(() => {
        if (fifthOriginArrivalInputVal != _selectedArrival5?.iataCode) {
            // Set the fifth origin destination city code
    ...
    const [flightSearchCredentials, setFlightSearchCredentials] = useState({
        originLocationCityCode1: flightRequest.originLocationCityCode1,
        originDestinationCityCode1: flightRequest.originDestinationCityCode || flightRequest.originDestinationCityCode1,
        departureDate1: flightRequest.departureDate1,
        arrivalDate1: flightRequest.arrivalDate1,
        originLocationCityCode2: flightRequest.originLocationCityCode2,
        originDestinationCityCode2: flightRequest.originDestinationCityCode2,
        departureDate2: flightRequest.departureDate2,
        originLocationCityCode3: flightRequest.originLocationCityCode3,
        originDestinationCityCode3: flightRequest.originDestinationCityCode3,
        departureDate3: flightRequest.departureDate3,
        originLocationCityCode4: flightRequest.originLocationCityCode4,
        originDestinationCityCode4: flightRequest.originDestinationCityCode4,
        departureDate4: flightRequest.departureDate4,
        originLocationCityCode5: flightRequest.originLocationCityCode5,
        originDestinationCityCode5: flightRequest.originDestinationCityCode5,
        departureDate5: flightRequest.departureDate5
    });
    ...
    function removeSearchRow() {
        ...
        for (let i = length; i >= 0; i--) {
            ...
            if (searchRows[i].active) {
                ...
                console.log('Search Rows Updated:', searchRows);
                // Exit the function
                return;
            }
        }
        ...
        await validateForms()
            .then(async (fields) => {
                ...
                if (routeModel == RouteModel.MultiCity) {
                    ...
                    let unSelectedRows = searchRows.filter((row) => row.active == false);
                    let totalNumberOfCities = selectedRows.length + 1;
                    let originDestinations = [];

                    // For each un selected row...
                    unSelectedRows.forEach(element => {

                        // Get the location key
                        let originLocationCityCode = `originLocationCityCode${element.id}`;

                        // Get the destination key
                        let originDestinationCityCode = `originDestinationCityCode${element.id}`;

                        // Set the location value to null
                        fields[originLocationCityCode] = null;

                        // Set the destination value to null
                        fields[originDestinationCityCode] = null;
                    });
            ...
            {!onMobile && (
                <>
                    <div className='container'  style={{padding: 0}}>
                        ...
                        onSubmit={async (e) => await onFlightSearch(e)}>
                            <h3><strong>Search and Book Flights</strong></h3>
                                ...
                                <div className='spEntryContainer__location'>
                                    ...
                                    <div className='locationContainer'>
                                        <input
                                            ...
                                            defaultValue={`${selectedDeparture.name} (${selectedDeparture.iataCode})`}
                                            onChange={async (e) => {
                                    ...
                                    <div className='locationContainer'>
                                        <input
                                            ...
                                            defaultValue={`${selectedArrival.name} (${selectedArrival.iataCode})`}
                                            onChange={async (e) => {
                                                let value = e.currentTarget.value;
                                                // If the field has no value...
                                                if (!value || value.length < 3) {
                                ...
                                <div className='spEntryContainer__date'>
                                    ...
                                    <DatePicker
                                        ...
                                        onChange={(date) => {
                                            ...
                                            setOriginDepartureDateInputVal(moment(date).format('yyyy-MM-DD'));
                                        }}
                                                ...
                                                <div className='spEntryContainer__location'>
                                                    ...
                                                    <div className='locationContainer'>
                                                        <input
                                                            ...
                                                            defaultValue={_selectedDeparture2 ? `${_selectedDeparture2?.name} (${_selectedDeparture2?.iataCode})` : null}
                                                    ...
                                                    <div className='locationContainer'>
                                                        <input
                                                            ...
                                                            defaultValue={_selectedArrival2 ? `${_selectedArrival2?.name} (${_selectedArrival2?.iataCode})` : null}
                                                            onChange={async (e) => {
                                                                let value = e.currentTarget.value;
                                                                // If the field has no value...
                                                                if (!value || value.length < 3) {
                                                ...
                                                <div className='spEntryContainer__location'>
                                                    ...
                                                    <div className='locationContainer'>
                                                        <input
                                                            ...
                                                            defaultValue={_selectedDeparture3 ? `${_selectedDeparture3?.name} (${_selectedDeparture3?.iataCode})` : null}
                                                            // onChange={(e) => { filterDepLocation(e) }} 
                                                            onChange={async (e) => {
                                                                let value = e.currentTarget.value;
                                                                // If the field has no value...
                                                    ...
                                                    <div className='locationContainer'>
                                                        <input
                                                            ...
                                                            defaultValue={_selectedArrival3 ? `${_selectedArrival3?.name} (${_selectedArrival3?.iataCode})` : null}
                                                            onChange={async (e) => {
                                                                let value = e.currentTarget.value;
                                                                // If the field has no value...
                                                                if (!value || value.length < 3) {
                                                ...
                                                <div className='spEntryContainer'>
                                                    ...
                                                    <div className='spEntryContainer__location'>
                                                        ...
                                                        <div className='locationContainer'>
                                                            <input
                                                                ...
                                                                defaultValue={_selectedDeparture4 ? `${_selectedDeparture4?.name} (${_selectedDeparture4?.iataCode})` : null}
                                                                // onChange={(e) => { filterDepLocation(e) }} 
                                                                onChange={async (e) => {
                                                                    let value = e.currentTarget.value;
                                                                    // If the field has no value...
                                                        ...
                                                        <div className='locationContainer'>
                                                            <input
                                                                ...
                                                                defaultValue={_selectedArrival4 ? `${_selectedArrival4?.name} (${_selectedArrival4?.iataCode})` : null}
                                                                onChange={async (e) => {
                                                                    let value = e.currentTarget.value;
                                                                    // If the field has no value...
                                                                    if (!value || value.length < 3) {
                                                    ...
                                                    <div className='spEntryContainer__date'>
                                                        ...
                                                        <DatePicker
                                                            ...
                                                            onChange={(date) => {
                                                                setFourthDepartureDate(date);
                                                                setFourthOriginDepartureDateInputVal(moment(date).format('yyyy-MM-DD'));
                                                            }}
                                                            calendarContainer={MyContainer}
                                                ...
                                                <div className='spEntryContainer'>
                                                    ...
                                                    <div className='spEntryContainer__location'>
                                                        ...
                                                        <div className='locationContainer'>
                                                            <input
                                                                ...
                                                                defaultValue={_selectedDeparture5 ? `${_selectedDeparture5?.name} (${_selectedDeparture5?.iataCode})` : null}
                                                                onChange={async (e) => {
                                                                    let value = e.currentTarget.value;
                                                                    // If the field has no value...
                                                                    if (!value || value.length < 3) {
                                                        ...
                                                        <div className='locationContainer'>
                                                            <input
                                                                ...
                                                                defaultValue={_selectedArrival5 ? `${_selectedArrival5?.name} (${_selectedArrival5?.iataCode})` : null}
                                                                onChange={async (e) => {
                                                                    let value = e.currentTarget.value;
                                                                    // If the field has no value...
                                                                    if (!value || value.length < 3) {
                                                    ...
                                                    <div className='spEntryContainer__date'>
                                                        ...
                                                        <DatePicker
                                                            ...
                                                            onChange={(date) => {
                                                                setFifthDepartureDate(date);
                                                                setFifthOriginDepartureDateInputVal(moment(date).format('yyyy-MM-DD'));
                                                            }}
                                                            calendarContainer={MyContainer}
                            ...
                            <div className='spOptionsContainer'>
                                ...
                                and a minimum of 2 search forms is visible */}
                                    ...
                                    <div className='spOptionsContainerOption'>
                                        <input type='checkbox' name='flight' checked={directFlightOnly} onChange={(e) => setDirectFlightOnly(e.currentTarget.checked)} />
                                    ...
                                    </div>
                                    {
                                        routeModel == RouteModel.MultiCity || routeModel == RouteModel.OneWay ? (
                                            ...
                                            <div className='spOptionsContainerOption'>
                                                <input type='checkbox' name='days' checked={dateWindow} onChange={(e) => setDateWindow(e.currentTarget.checked)} />
                                                <div className='spOptionsContainerOption__checker'>
                                                    <BsCheck />
                                                </div>
                                                <p>+/-3 Days</p>
                    ...
                    <div className='mobileContainer'>
                        <div className='container'>
                            ...
                            <form className='mSearchPanelContainer' onSubmit={async (e) => await onFlightSearch(e)}>
                                                ...
                                                <div className='mSearchPanelContainer__mSetEntryData'>
                                                    ...
                                                    <div className='date'>
                                                        ...
                                                        <DatePicker
                                                            ...
                                                            onChange={(date) => {
                                                                setSecondDepartureDate(date);
                                                                setSecondOriginDepartureDateInputVal(moment(date).format('yyyy-MM-DD'));
                                                            }}
                                                            calendarContainer={MyContainer}
                                                            minDate={new Date()}
                                                            ref={secondOriginDepartureDateInput}
                                ...
                                <div className='mSearchPanelContainer__mSpOptionsContainer'>
                                    {
                                        routeModel === RouteModel.MultiCity &&
                                        (
                                            <div className='left'>
...
export default SearchPanelModify;
