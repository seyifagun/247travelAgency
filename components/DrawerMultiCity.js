...
const MultiCityTrial = ({
        ...
        .then(async () => {
          ...
          await fetchFlightResultsMultiCity(
            ...
            .then((response) => {
              ...
              router.push(
                "/flight-results-redirect?flightRequestId=" +
  ...
  const [showDummyElement, setShowDummyElement] = useState(false);

  // Search rows state
  const [searchRows, setSearchRows] = useState(_searchRows);
  ...
  useEffect(() => {
    ...
    var flightRequest = JSON.parse(searchMeta.flightRequest);

    // Set fields value

    // Get the first origin airport
    var originAirport = airports.find(
      (airport) => airport.iataCode == flightRequest.originLocationCityCode1
    );
    // Set the first destination airport
    var destinationAirport = airports.find(
      (airport) => airport.iataCode == flightRequest.originDestinationCityCode1
    );

    // Set the first origin airport name
    originLocationInput1.current.state.value = originAirport.name;
    // Set the first destination airport name
    originDestinationInput1.current.state.value = destinationAirport.name;

    // Set the fields value
    flightSearchForm.setFieldsValue({
      originLocationCityCode1: originAirport.iataCode,
      originDestinationCityCode1: destinationAirport.iataCode,
      departureDate1: flightRequest.departureDate1,
      selectedDepartureDate1: moment(flightRequest.selectedDepartureDate1),
      flightClass: flightRequest.flightClass,
      numberOfAdults: flightRequest.numberOfAdults,
      numberOfChildren: flightRequest.numberOfChildren,
      numberOfInfants: flightRequest.numberOfInfants,
      directFlightOnly: flightRequest.directFlightOnly,
    });

    setAdultNumValue(flightRequest.numberOfAdults);
    setChildrenNumValue(flightRequest.numberOfChildren);
    setInfantsNumValue(flightRequest.numberOfInfants);

    // If second row was active...
    if (searchRows[0].active) {
      // Get the origin location airport
      var originAirport = airports.find(
        (airport) => airport.iataCode == flightRequest.originLocationCityCode2
      );

      // Get the origin destination airport
      var destinationAirport = airports.find(
        (airport) =>
          airport.iataCode == flightRequest.originDestinationCityCode2
      );

      // Set the origin airport name
      originLocationInput2.current.state.value = originAirport.name;
      // Set the origin destination airport name
      originDestinationInput2.current.state.value = destinationAirport.name;

      // Set the fields value
      flightSearchForm.setFieldsValue({
        originLocationCityCode2: originAirport.iataCode,
        originDestinationCityCode2: destinationAirport.iataCode,
        departureDate2: flightRequest.departureDate2,
        selectedDepartureDate2: moment(flightRequest.selectedDepartureDate2),
      });
    }

    // If the third row is active...
    if (searchRows[1].active) {
      // Get the origin location airport
      var originAirport = airports.find(
        (airport) => airport.iataCode == flightRequest.originLocationCityCode3
      );

      // Get the origin destination airport
      var destinationAirport = airports.find(
        (airport) =>
          airport.iataCode == flightRequest.originDestinationCityCode3
      );

      // Set the origin airport name
      originLocationInput3.current.state.value = originAirport.name;
      // Set the origin destination airport name
      originDestinationInput3.current.state.value = destinationAirport.name;

      // Set the fields value
      flightSearchForm.setFieldsValue({
        originLocationCityCode3: originAirport.iataCode,
        originDestinationCityCode3: destinationAirport.iataCode,
        departureDate3: flightRequest.departureDate3,
        selectedDepartureDate3: moment(flightRequest.selectedDepartureDate3),
      });
    }

    // If the fourth row is active...
    if (searchRows[2].active) {
      // Get the origin location airport
      var originAirport = airports.find(
        (airport) => airport.iataCode == flightRequest.originLocationCityCode4
      );

      // Get the origin destination airport
      var destinationAirport = airports.find(
        (airport) =>
          airport.iataCode == flightRequest.originDestinationCityCode4
      );

      // Set the origin airport name
      originLocationInput4.current.state.value = originAirport.name;
      // Set the origin destination airport name
      originDestinationInput4.current.state.value = destinationAirport.name;

      // Set the fields value
      flightSearchForm.setFieldsValue({
        originLocationCityCode4: originAirport.iataCode,
        originDestinationCityCode4: destinationAirport.iataCode,
        departureDate4: flightRequest.departureDate4,
        selectedDepartureDate4: moment(flightRequest.selectedDepartureDate4),
      });
    }

    // If the fifth row is active...
    if (searchRows[3].active) {
      // Get the origin location airport
      var originAirport = airports.find(
        (airport) => airport.iataCode == flightRequest.originLocationCityCode5
      );

      // Get the origin destination airport
      var destinationAirport = airports.find(
        (airport) =>
          airport.iataCode == flightRequest.originDestinationCityCode5
      );

      // Set the origin airport name
      originLocationInput5.current.state.value = originAirport.name;
      // Set the origin destination airport name
      originDestinationInput5.current.state.value = destinationAirport.name;

      // Set the fields value
      flightSearchForm.setFieldsValue({
        originLocationCityCode5: originAirport.iataCode,
        originDestinationCityCode5: destinationAirport.iataCode,
        departureDate5: flightRequest.departureDate5,
        selectedDepartureDate5: moment(flightRequest.selectedDepartureDate5),
      });
    }

    setShowDummyElement(true);

    console.log("Fields Values:", flightSearchForm.getFieldsValue());
  }, []);
              ...
              <div className="col-md-6 col-lg-3">
                <div className="flight">
                  <div className="flight-booking-index">
                    ...
                    <FormAnt.Item
                      name="flightClass"
                      initialValue={flightRequestJson.flightClass}
                    >
            ...
            <div className="addflight-div">
              ...
              >
                <FormAnt.Item
                  ...
                  initialValue={directFlightOnly ? directFlightOnly : false}
                >
                  <Checkbox
